...
class ApiAccess extends Page
{
    ...
    protected function getViewData(): array
    {
        ...
        $activeCount = $tokens->filter(fn (ApiAccessToken $token): bool => $token->isUsable())->count();
        $revokedCount = $tokens->filter(fn (ApiAccessToken $token): bool => $token->isRevoked())->count();
        $expiredCount = $tokens->filter(fn (ApiAccessToken $token): bool => $token->isExpired() && ! $token->isRevoked())->count();
